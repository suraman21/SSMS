...
(async () => {
    ...
    // Client minimum boundaries reject either undersized axis and accept 64x64.
    for (const [width, height] of [[63, 64], [64, 63], [63, 63]]) {
        nextImageDimensions = { width, height, fail: false };
        await assert.rejects(
            hooks.inspectSelectedImage({ type: 'image/png', size: 100 }),
            /at least 64 × 64/
        );
    }
    nextImageDimensions = { width: 64, height: 64, fail: false };
    const minimumPreview = await hooks.inspectSelectedImage({ type: 'image/png', size: 100 });
    assert.match(minimumPreview, /^blob:test-/);
    TestURL.revokeObjectURL(minimumPreview);
