...
def test_every_supported_authenticated_role_reaches_one_common_page():
    ...
    assert "components/account_link.php" in dashboard
    for shell in ("frontend/pages/finance_dept.php", "frontend/pages/mezmur_dept.php"):
        shell_source = text(ROOT / shell)
        assert shell_source.count("ssms_app_url('admin/account.php')") == 2
