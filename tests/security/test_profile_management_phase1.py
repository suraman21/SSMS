...
class ProfileManagementPhase1Tests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.web_api = (ROOT / "admin/api_settings.php").read_text(encoding="utf-8")
        cls.mobile_api = (ROOT / "api/v1/routes/users.php").read_text(encoding="utf-8")
        cls.cleanup = (ROOT / "admin/backend/profile_image_cleanup.php").read_text(
            encoding="utf-8"
        )
        cls.runbook = (ROOT / "docs/audits/DEPLOYMENT_RUNBOOK.md").read_text(
            encoding="utf-8"
        )
    ...
    def test_php_behavior_fixture(self):
        ...
        if result["image"]["available"]:
            ...
            self.assertTrue(result["image"]["minimum_dimensions"], result)
            ...
            self.assertTrue(result["image"]["orphan_reaper"], result)
    ...
    def test_profile_version_is_mandatory_and_never_manufactured_by_controllers(self):
        self.assertIn("function requireProfileVersion", self.profile)
        self.assertIn("/^[a-f0-9]{64}$/D", self.profile)
        self.assertIn("$expectedVersion = self::requireProfileVersion", self.profile)
        self.assertIn("function settingsRequireProfileVersion", self.web_api)
        self.assertIn("$expectedVersion = settingsRequireProfileVersion($input);", self.web_api)
        self.assertIn("'PROFILE_VERSION_REQUIRED'", self.web_api)
        web_update = self.web_api.split("case 'profile_update':", 1)[1].split(
            "case 'password_change':", 1
        )[0]
        self.assertLess(
            web_update.index("settingsRequireProfileVersion($input)"),
            web_update.index("getOwnProfile($settingsIdentity)"),
        )
        self.assertNotIn("$before['profile_version']", self.web_api)
        self.assertNotIn("use the just-read version", self.web_api)
        self.assertIn("function usersApiRequireProfileVersion", self.mobile_api)
        self.assertGreaterEqual(
            self.mobile_api.count("usersApiRequireProfileVersion("), 4
        )
    ...
    def test_orphan_cleanup_is_server_referenced_aged_confined_and_lock_safe(self):
        for contract in (
            "REAP_MINIMUM_AGE_SECONDS = 86400",
            "function reapUnreferenced",
            "referencedLogicalReferences",
            "FilesystemIterator::SKIP_DOTS",
            "LOCK_EX | LOCK_NB",
            "acquireCleanupLock(LOCK_SH)",
            "dirname($real) !== $root",
            "isFinal && isset($referencedNames[$name])",
            "TEMPORARY_NAME_PATTERN",
        ):
            self.assertIn(contract, self.image)
        self.assertIn("PHP_SAPI !== 'cli'", self.cleanup)
        self.assertIn("referencedImageReferences()", self.cleanup)
        self.assertIn("reapUnreferenced(", self.cleanup)
        self.assertNotIn("$_GET", self.cleanup)
        self.assertNotIn("$_POST", self.cleanup)

    def test_cleanup_is_not_invoked_by_normal_web_or_auth_requests(self):
        request_paths = (
        ...
        for relative in request_paths:
            ...
            self.assertNotIn("profile_image_cleanup.php", source, relative)
            self.assertNotIn("reapUnreferenced(", source, relative)
