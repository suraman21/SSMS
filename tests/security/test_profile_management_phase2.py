...
class ProfileManagementPhase2Tests(unittest.TestCase):
    ...
    def test_profile_version_is_returned_by_get_and_required_only_for_mutations(self):
        image_route = self.users.split(
            "// /users/me/profile-image", 1
        )[1].split("// GET /users/me", 1)[0]
        image_get = image_route.split("if ($method === 'GET')", 1)[1].split(
            "$ip =", 1
        )[0]
        self.assertNotIn("usersApiRequireProfileVersion", image_get)
        self.assertIn("usersApiRequireProfileVersion($_POST)", image_route)
        self.assertIn("usersApiRequireProfileVersion($body)", image_route)

        profile_get = self.users.split("// GET /users/me", 1)[1].split(
            "// PATCH /users/me", 1
        )[0]
        profile_patch = self.users.split("// PATCH /users/me", 1)[1].split(
            "// POST /users/change-password", 1
        )[0]
        self.assertNotIn("usersApiRequireProfileVersion", profile_get)
        self.assertIn("usersApiRequireProfileVersion($body)", profile_patch)

        web_get = self.settings.split("case 'profile_get':", 1)[1].split(
            "case 'profile_update':", 1
        )[0]
        web_update = self.settings.split("case 'profile_update':", 1)[1].split(
            "case 'password_change':", 1
        )[0]
        self.assertNotIn("settingsRequireProfileVersion", web_get)
        self.assertIn("settingsRequireProfileVersion($input)", web_update)
