...
class FakeGateway implements ProfileGateway {
  ...
  Future<ApiResponse> removeImage(String profileVersion) async {
    removedVersion = profileVersion;
    return removalResponses.removeAt(0);
  }
...
void main() {
  ...
  test('account switching cannot reuse the previous owner profile version',
      () async {
    createService();
    gateway.cached = canonicalProfile(version: profileVersionA);
    await service.open();
    ...
    gateway.updateResponses.add(ok(canonicalProfile(
      id: 8,
      username: 'second.user',
      fullName: 'Second Updated',
      version: profileVersionA,
    )));

    final result = await service.updateFullName('Second Updated');

    expect(result.success, isTrue);
    expect(gateway.updates.single['profile_version'], profileVersionB);
  });
  ...
  test('failed image upload and removal preserve the old avatar', () async {
    ...
    expect(result.success, isFalse);
    expect(gateway.removedVersion, profileVersionA);
    expect(service.imageBytes, orderedEquals(oldImage));
