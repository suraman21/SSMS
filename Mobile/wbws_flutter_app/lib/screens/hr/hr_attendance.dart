...
/// HR department attendance — VERBATIM clone of the mezmur/teachers'
/// attendance workflow, keyed by (date, section):
///
///   [Section ▾] + [Ethiopian date] → that section's member list →
///   P/A/L/E chips + tap-for-note → Save (draft) / Submit (packet to
///   the HR department) → HR approves or returns it with a note.
///
/// Isolation rule: HR attendance is HR's OWN domain (hr_attendance /
/// hr_submissions on the server, pending_hr / cached_hr_sheet on the
/// phone). It never mixes with Education or Mezmur data.
///
/// Offline parity: SQLite outbox (pending_hr), sheet cache keyed by
/// date+section, background sync with idempotency keys.
class HrAttendanceScreen extends StatefulWidget {
  final String? initialDate;
  final String? initialSection;
  const HrAttendanceScreen({super.key, this.initialDate, this.initialSection});
  @override
  State<HrAttendanceScreen> createState() => HrAttendanceScreenState();
}

class HrAttendanceScreenState extends State<HrAttendanceScreen> {
  ...
  /// Roster of the selected section.
  ...
  Future<void> _updatePendingCount() async {
    final count = await _db.getPendingHrCount();
    if (mounted) setState(() => _pendingCount = count);
  }

  // ── [Section ▾] like teachers' [Class ▾] ─────────────────────
  Future<void> _loadSections() async {
    final cached = await _db.getCachedHrSections();
    ...
    if (res.success && res.data != null) {
      ...
      if (list.isNotEmpty) {
        _sections = list;
        await _db.cacheHrSections(list);
        setState(() {
          _loadingSections = false;
          _isOffline = !ConnectivityService().hasLink;
        });
  ...
  Future<void> _loadSheet() async {
    ...
    final cached = await _db.getCachedHrSheet(_selectedDate, section);
    ...
    if (cacheLocked && mounted) {
      ...
      await _db.dropPendingHr(_selectedDate, section);
    }
    ...
    // 3. Fresh fetch
    final res = await _api.getHrSheet(_selectedDate, section: section);
    if (!mounted) return;
    if (res.success && res.data != null) {
      final data = res.data!;
      // Version handshake: the current server stamps every HR sheet
      // response with 'v'. Missing marker => stale backend.
      _staleServer = data['v'] == null;
      ...
      _applySheet(
          ...
          locked ? {} : pendingNotes,
          locked);
      await _db.cacheHrSheet(_selectedDate, section, data);
      setState(() {
        _loading = false;
        _rosterReady = true;
        _loadFailed = false;
        ...
        _returnNote = _hrReturnNote(data);
      });
    } else if (_members.isEmpty) {
      var msg = res.message ?? 'Could not load the sheet.';
      if (RegExp(r'server error', caseSensitive: false).hasMatch(msg)) {
        msg +=
            ' The server may be outdated — ask the administrator to pull the latest code and run sql/026_hr_attendance.sql.';
      }
      setState(() {
        _loading = false;
        _rosterReady = true;
  ...
  /// correction — the taker's key back to editing.
  String? _hrReturnNote(Map<String, dynamic> data) {
    ...
    final by = reviewer.isEmpty ? 'The HR department' : reviewer;
  ...
  Future<void> _persistLocal() async {
    ...
    await _db.cacheHrSheet(_selectedDate, section, {
      'date': _selectedDate,
      'section': section,
      'members': _members,
  ...
  Future<void> _autoSaveNow() async {
    ...
    try {
      await _db.saveHrLocal(_selectedDate, section, marked,
          packetKind: 'draft');
    } catch (_) {
      return;
    }
  ...
  void _openQrScan() {
    QrScanSheet.open(context,
        onScan: _handleQrScan, header: _selectedSection ?? 'HR');
  }

  /// HR rules for one scan: member must belong to the open section;
  /// duplicates and the packet lock are refused with big Amharic
  /// feedback; success marks Present and autosaves at once.
        ...
        children: [
          ...
          if (_staleServer && _error == null)
            StatusBanner.warning(
                'This server is running an older version of the HR attendance backend. Ask the administrator to pull the latest code and run sql/026_hr_attendance.sql.'),
          if (_returnNote != null &&
              _returnNote!.isNotEmpty &&
              !_locked &&
              _error == null)
            StatusBanner.warning(
                'Returned by the HR department — you can edit and resubmit. $_returnNote'),
