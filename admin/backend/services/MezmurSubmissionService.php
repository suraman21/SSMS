...
/**
 * ════════════════════════════════════════════════════════════
 * MezmurSubmissionService — submission packets for mezmur
 * attendance (መዝሙር ክፍል). Exact workflow clone of
 * SubmissionService (teachers → education), scoped by
 * (attendance_date, section) instead of (class, date):
 *
 *   taker app → save/submit packet → mezmur dept reviews
 *   (approve / reject / return-with-note) → taker edits &
 *   resubmits.
 *
 * Status vocabulary mirrors grade_submissions exactly:
 *   draft / incomplete / submitted / approved / rejected /
 *   revision_needed.
 *
 * All methods take an open \mysqli and use prepared statements;
 * exceptions never leak internals (callers translate).
 * ════════════════════════════════════════════════════════════
 */
...
final class MezmurSubmissionService
{
    ...
    public static function upsert(\mysqli $conn, array $opts): array
    {
        ...
        if ($existingId > 0) {
            $cur = $conn->prepare("SELECT status FROM mezmur_submissions WHERE id = ? LIMIT 1");
            $curStatus = '';
            if ($cur) {
            ...
            return [
                ...
                'message' => $status === self::STATUS_SUBMITTED
                    ? 'Attendance submitted to the Mezmur department.'
                    : 'Saved as a draft for the Mezmur department.',
            ];
        }
    ...
    // ── review (mezmur dept + admins only; callers gate roles) ──
